...
A short-lived server-side session token used to call GET methods.
...
- keyed by app user context (for stored-credential mode)
- clear cache immediately on auth failures

Important deployment note:

- in-memory cache is process-local only
- distributed deployments need shared cache if cross-instance reuse is required
